...
const DigitalSilverScreen = () => {
  const navigation = useNavigation() as any;
  const { mode } = useThemeStore();
  const colors = mode === 'dark' ? Colors.dark : Colors.light;
  const styles = getStyles(colors, mode);

  const [activeTab, setActiveTab] = useState<'silver'>('silver');
  ...
  const handleBuy = () => {
    ...
    if (hours < 11 || hours >= 18) {
      Alert.alert(
        "Market Closed",
        "Digital Silver purchases are only allowed between 11:00 AM and 6:00 PM IST based on live market hours."
      );
      return;
    }
        ...
        <View style={[styles.inputCard, { backgroundColor: colors.cardBackground, shadowColor: mode === 'dark' ? '#000' : COLORS.black }]}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => Alert.alert(
              ...
              "2. The equivalent silver weight or amount will be automatically calculated based on the live market rate.\n\n" +
              "3. Click 'Buy Now' to proceed to the secure payment gateway.\n\n" +
              "4. Once purchased, the digital silver will be instantly credited to your secure Digital Locker.\n\n" +
              "5. You can redeem your accumulated digital silver for physical items at our NS MAHAVEER JEWELLERY showroom at any time!"
            )}
          >
          ...
          {amount && parseFloat(amount) > 0 ? (
            <View style={[styles.summaryBox, { backgroundColor: mode === 'dark' ? 'rgba(255,255,255,0.05)' : '#F9F9F9' }]}>
              ...
              <View style={styles.summaryRow}>
                <Text style={[styles.summaryLabel, { color: colors.textMuted }]}>Silver Value</Text>
...
const getStyles = (colors: any, mode: string) => StyleSheet.create({
  ...
  metalIconActive: {
    backgroundColor: 'rgba(255, 255, 255, 0.15)', // Silver accent
...
export default DigitalSilverScreen;
