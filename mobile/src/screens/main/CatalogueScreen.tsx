...
const CatalogueScreen = () => {
    ...
    return (
      <TouchableOpacity key={item.id} style={styles.categoryCard}>
        <View style={[styles.categoryIconContainer, { backgroundColor: colors.cardBackground, shadowColor: mode === 'dark' ? '#000' : COLORS.black }]}>
          {item.image ? (
            <Image source={{uri: `http://10.115.217.171:5000${item.image}`}} style={styles.categoryImg} />
          ) : (
  ...
  const renderJewelleryItem = ({ item, index }: any) => {
    return (
      ...
      >
        <Image 
          source={{ uri: item.images?.[0] ? `http://10.115.217.171:5000${item.images[0]}` : 'https://via.placeholder.com/200' }} 
