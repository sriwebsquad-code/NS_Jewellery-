  ...
  try {
    ...
    if (type === 'transactions') {
      if (category === 'digigold' || category === 'digisilver') {
        ...
        let schemeType = '';
        let metalType = '';
        if (category === 'gold_value') { schemeType = 'VALUE_BASED'; metalType = 'GOLD'; }
        else if (category === 'silver_value') { schemeType = 'VALUE_BASED'; metalType = 'SILVER'; }
        else if (category === 'gold_weight') { schemeType = 'WEIGHT_BASED'; metalType = 'GOLD'; }
        else if (category === 'silver_weight') { schemeType = 'WEIGHT_BASED'; metalType = 'SILVER'; }
        ...
        if (planIds.length > 0) {
          ...
          if (planIds.length <= 10) {
            userPlansSnap = await db.collection('userPlans').where('planId', 'in', planIds).get();
          } else {
            userPlansSnap = await db.collection('userPlans').get();
          }
      ...
      if (category === 'digigold' || category === 'digisilver') {
        ...
        const userTotals: Record<string, { amount: number, weight: number }> = {};
