...
async function fixDigitalBalances() {
    ...
    const txnSnap = await firebase_1.db.collection("digitalTransactions")
    ...
    let count = 0;
    for (const uid in balanceMap) {
        const oldDoc = await firebase_1.db.collection("digitalBalances").doc(uid).get();
        ...
        await firebase_1.db.collection("digitalBalances").doc(uid).set({ goldBalance: newGold, silverBalance: newSilver }, { merge: true });
        count++;
    }
...
//# sourceMappingURL=fix_balances.js.map